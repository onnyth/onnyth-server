...
class UserControllerTest {
    ...
    class GetAllUsers {
        ...
        void returns200_whenAuthenticated() throws Exception {
            ...
            when(userUseCaseService.getAllUsers()).thenReturn(List.of(user));
    ...
    class GetUserById {
        ...
        void returns200_whenFound() throws Exception {
            User user = TestDataFactory.aUserWithId(USER_ID);
            when(userUseCaseService.getUserById(USER_ID)).thenReturn(Optional.of(user));
        ...
        void returns404_whenNotFound() throws Exception {
            when(userUseCaseService.getUserById(USER_ID)).thenReturn(Optional.empty());
    ...
    class GetUserByEmail {
        ...
        void returns200_whenFound() throws Exception {
            ...
            when(userUseCaseService.getUserByEmail("test@example.com")).thenReturn(Optional.of(user));
        ...
        void returns404_whenNotFound() throws Exception {
            when(userUseCaseService.getUserByEmail("unknown@example.com")).thenReturn(Optional.empty());
    ...
    class CreateUser {
        ...
        void returns200_onSuccess() throws Exception {
            User user = TestDataFactory.aUserWithId(USER_ID);
            when(userUseCaseService.createUser(any(User.class))).thenReturn(user);
    ...
    class DeleteUser {
        ...
        void returns204_onSuccess() throws Exception {
            doNothing().when(userUseCaseService).deleteUser(USER_ID);
